...
namespace Runner.Scripts.Controller
{
    public class FloorManager : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            float tilePositionHorizontalOffset = (floorSize.x * 2.5f) - LevelManager.horizontalScreenSize;
            float tilePositionVerticalOffset = (floorSize.y * 2f) - LevelManager.verticalScreenSize;

            float sumGameObjectHorizontalSize = 0f;

            // Added two more tiles to guarantee that the replacement does not show on screen
            float totalScreenSize = (LevelManager.horizontalScreenSize + FloorPrefabHorizontalSize) * 2f;
